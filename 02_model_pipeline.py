...
def scale_features(X_train, X_test, feature_cols):
    ...
    for col in feature_cols[:5]:  # Show first 5 features
        print(
            f"{col:<15} {X_train_scaled[col].mean():>12.4f} {X_train_scaled[col].std():>12.4f} "
...
def feature_engineering_pipeline(
    ...
    paths = save_engineered_data(
        X_train_scaled, X_test_scaled, y_train, y_test, scaler, feature_cols, output_dir
    )
...
def train_logistic_regression(X_train, y_train, cv=5):
    ...
    if FAST_MODE:
        param_grid = {"C": [0.1, 1], "penalty": ["l2"], "solver": ["liblinear"], "max_iter": [500]}
    else:
        param_grid = {
    ...
    grid_search = GridSearchCV(lr, param_grid, cv=cv, scoring="roc_auc", n_jobs=N_JOBS, verbose=1)
...
def train_svm(X_train, y_train, cv=5):
    ...
    print("\n[INFO] Configuring hyperparameter grid...")
    if FAST_MODE:
        param_grid = {"C": [0.1, 1], "kernel": ["rbf"], "gamma": ["scale", 0.001]}
    else:
        param_grid = {
            "C": [0.1, 1, 10, 100],
            "kernel": ["rbf", "linear"],
    ...
    grid_search = GridSearchCV(svm, param_grid, cv=cv, scoring="roc_auc", n_jobs=N_JOBS, verbose=1)
...
def train_gradient_boosting(X_train, y_train, cv=5):
    ...
    grid_search = GridSearchCV(gb, param_grid, cv=cv, scoring="roc_auc", n_jobs=N_JOBS, verbose=1)
...
def train_all_models(X_train, y_train, cv=5, output_dir="models"):
    ...
    models["logistic_regression"] = {"model": lr_model, "params": lr_params, "cv_score": lr_score}
    ...
    models["random_forest"] = {"model": rf_model, "params": rf_params, "cv_score": rf_score}

    # SVM
    svm_model, svm_params, svm_score = train_svm(X_train, y_train, cv)
    save_model(svm_model, "svm", svm_params, svm_score, output_dir)
    models["svm"] = {"model": svm_model, "params": svm_params, "cv_score": svm_score}
    ...
    models["gradient_boosting"] = {"model": gb_model, "params": gb_params, "cv_score": gb_score}

    return models


def model_training_pipeline(data_dir="data/engineered", output_dir="models", cv=DEFAULT_CV):
    """
...
def load_data_and_models(data_dir="data/engineered", models_dir="models"):
    ...
    try:
        ...
        # If import fails, continue and let joblib raise a clear
        # error when loading
        pass
...
def plot_metrics_comparison(all_results, output_dir):
    ...
    # Prepare data
    models = []
    metrics_data = {"Accuracy": [], "Precision": [], "Recall": [], "F1-Score": [], "ROC-AUC": []}
...
def model_evaluation_pipeline(
    data_dir="data/engineered", models_dir="models", output_dir="outputs/model_evaluation"
):
    """
