...
def plot_correlation_heatmap(df, output_dir):
    ...
    sns.heatmap(
        corr_matrix, mask=mask, annot=True, fmt=".2f", cmap="coolwarm", center=0, square=True
    )
    plt.tight_layout()
...
def run_eda_pipeline(
    data_path="data/processed/heart_disease_processed.csv", output_dir="outputs/eda_visualizations"
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Combined data pipeline (download, preprocess, eda)"
    )
    ...
    parser.add_argument(
        "--all", action="store_true", help="Run all steps: download -> preprocess -> eda"
    )
