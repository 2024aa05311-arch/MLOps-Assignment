...
def train_and_log_model(model_name, model, param_grid, X_train, y_train, X_test, y_test, cv=5):
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        print(f"\n[METRICS]")
        print(f"  Training - Accuracy: {metrics['train_accuracy']:.4f}, ROC-AUC: {metrics['train_roc_auc']:.4f}")
        print(f"  Testing  - Accuracy: {metrics['test_accuracy']:.4f}, ROC-AUC: {metrics['test_roc_auc']:.4f}")
...
def run_all_mlflow_experiments(data_dir='data/engineered', cv=5):
    ...
    best_model_name = max(results.items(), 
                         key=lambda x: x[1]['metrics']['test_roc_auc'])[0]
